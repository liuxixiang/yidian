...
//        }
    ...
    public static void setRequestedOrientation(Context context, int orientation) {
        if (YdMediaUtils.getAppCompActivity(context) != null) {
            YdMediaUtils.getAppCompActivity(context).setRequestedOrientation(
                    orientation);
        } else {
            YdMediaUtils.scanForActivity(context).setRequestedOrientation(
                    orientation);
        }
    }
    ...
    public static Window getWindow(Context context) {
        if (YdMediaUtils.getAppCompActivity(context) != null) {
            return YdMediaUtils.getAppCompActivity(context).getWindow();
        } else {
            return YdMediaUtils.scanForActivity(context).getWindow();
        }
    }
    ...
    public static void saveProgress(Context context, Object url, long progress) {
        if (!YdVideoPlayer.SAVE_PROGRESS) return;
    ...
    public static long getSavedProgress(Context context, Object url) {
        if (!YdVideoPlayer.SAVE_PROGRESS) return 0;
        SharedPreferences spn = context.getSharedPreferences("JZVD_PROGRESS",
                Context.MODE_PRIVATE);
