...
//    }
    ...
    private static void showCustomizedToast(String s, boolean succ) {
        if (android.os.Build.VERSION.SDK_INT > 10) {
            try {
                View toastView = LayoutInflater.from(sAppContext).inflate(R.layout.sdk_customized_toast_layout, null);
                TextView tv = (TextView) toastView.findViewById(R.id.toastTxt);
                tv.setText(s);
                if (succ) {
                    tv.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.sdk_notice_succeed, 0, 0, 0);
                } else {
                    tv.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.sdk_notice_failure, 0, 0, 0);
                }
